...
async function sendTestEmail() {
  console.log('Sending professional Mini PIA campaign test to info@1402celsius.com...\n');
  ...
  try {
    const { data, error } = await resend.emails.send({
      ...
      html: emailTemplate,
      tags: [
        { name: 'campaign', value: 'minipia-professional-test' }
      ]
    });
    ...
    if (error) {
      ...
      console.log('✅ Professional template test email sent successfully!');
      console.log('Email ID:', data.id);
      console.log('\nEmail details:');
      console.log('- Subject: "Unlock 50% Grant Funding for Your Italian Dream"');
      console.log('- From: Giuseppe Funaro <hello@investinpuglia.eu>');
      console.log('- Greeting: "Dear Investor" (generic)');
      console.log('- Includes all professional content from yesterday\'s template');
    }
  } catch (error) {
