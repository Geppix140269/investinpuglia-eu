...
import { MapPin, Euro, Calendar, ExternalLink, Image as ImageIcon, Save, X, Upload, Plus, Edit2, Check } from 'lucide-react'
...
const INITIAL_CV_PROJECTS = {
  ...
  "public": [
    {
      id: "comparto-c3",
      name: "Lottizzazione Comparto C3.1",
      location: "Otranto",
      budget: "N/A",
      completion: "2007-2015",
      description: "Urban plan approved"
    },
    {
      id: "lungomare",
      name: "Lungomare Idro",
      ...
      description: "Seafront redevelopment"
    },
    {
      id: "asilo-ruffano",
      name: "Asilo Nido",
      ...
      budget: "€1.5M",
      completion: "2024",
      description: "PNRR funded nursery school"
    },
    {
      id: "schools-energy",
      name: "Efficientamento Energetico",
      location: "Otranto, Lecce, Alessano",
      budget: "€1.6M",
      completion: "2016-2022",
      description: "Schools energy efficiency upgrades"
    }
  ]
}
...
export default function PortfolioEditor() {
  ...
  useEffect(() => {
    ...
    if (saved) {
      setImageAssignments(JSON.parse(saved))
    }
    ...
    if (savedProjects) {
      setCvProjects(JSON.parse(savedProjects))
    }
  ...
  const assignImage = (projectId: string, imageUrl: string) => {
    ...
    setSavedMessage('Image assigned successfully!')
  ...
  const handleImageUpload = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    Array.from(files).forEach(file => {
      ...
      reader.onload = (e) => {
        ...
        setSavedMessage(`Uploaded ${file.name}`)
        setTimeout(() => setSavedMessage(''), 3000)
      }
  ...
  const clearUploadedImages = () => {
    if (confirm('Are you sure you want to clear all uploaded images?')) {
      ...
      setSavedMessage('Cleared all uploaded images')
      setTimeout(() => setSavedMessage(''), 3000)
    }
  }

  const startEditingProject = (category: string, projectId: string) => {
    const project = cvProjects[category].find((p: any) => p.id === projectId)
    if (project) {
      setEditingProject(projectId)
      setEditedProject({ ...project, category })
    }
  }
  ...
  const saveProjectEdit = () => {
    ...
    const categoryProjects = newProjects[editedProject.category]
    const projectIndex = categoryProjects.findIndex((p: any) => p.id === editingProject)
    
    if (projectIndex !== -1) {
      categoryProjects[projectIndex] = {
        ...editedProject,
        id: editingProject
      }
      delete categoryProjects[projectIndex].category
  ...
  const resetProjects = () => {
    if (confirm('Are you sure you want to reset all projects to original CV data? This will lose all your edits.')) {
      setCvProjects(INITIAL_CV_PROJECTS)
      localStorage.removeItem('portfolio-projects')
      setSavedMessage('Projects reset to original data')
      setTimeout(() => setSavedMessage(''), 3000)
    }
  }
  ...
  const exportAssignments = () => {
    ...
    link.setAttribute('download', 'portfolio-complete-data.json')
    link.click()
  }
    ...
    <div className="min-h-screen bg-gray-50 py-8">
      <div className="container mx-auto px-4">
        <div className="bg-white rounded-xl shadow-lg p-8 mb-8">
          <h1 className="text-3xl font-bold mb-4">Portfolio Image Manager</h1>
          <p className="text-gray-600 mb-6">
            Assign images to projects from Cataldo Russo's CV. Click on any project to assign or change its image.
          </p>
        ...
        <div className="mb-12">
          <h2 className="text-2xl font-bold mb-6 text-indigo-900">Hospitality & Tourism Structures</h2>
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
            {cvProjects.hospitality.map((project: any) => (
              <div key={project.id} className="bg-white rounded-lg shadow-md overflow-hidden hover:shadow-xl transition-all">
                <div 
                ...
                >
                  {imageAssignments[project.id] ? (
                    <>
                      <img
                        src={imageAssignments[project.id]}
                        alt={project.name}
                  ...
                  {editingProject === project.id ? (
                    ...
                    <div className="space-y-3">
                      ...
                      <input
                        ...
                        onChange={(e) => setEditedProject({...editedProject, completion: e.target.value})}
                        className="w-full px-2 py-1 border rounded text-sm"
                    ...
                    <>
                      <div className="flex justify-between items-start">
                        ...
                        <button
                          onClick={() => startEditingProject('hospitality', project.id)}
        ...
        <div className="mb-12">
          <h2 className="text-2xl font-bold mb-6 text-indigo-900">Heritage & Restoration</h2>
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
            {cvProjects.heritage.map((project: any) => (
              <div key={project.id} className="bg-white rounded-lg shadow-md overflow-hidden hover:shadow-xl transition-all">
                <div 
                  className="relative h-48 bg-gray-200 cursor-pointer group"
                  onClick={() => {
        ...
        <div className="mb-12">
          <h2 className="text-2xl font-bold mb-6 text-indigo-900">Public Works & Urban Planning</h2>
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
            {CV_PROJECTS.public.map((project) => (
              <div key={project.id} className="bg-white rounded-lg shadow-md overflow-hidden hover:shadow-xl transition-all">
                <div 
                  className="relative h-48 bg-gray-200 cursor-pointer group"
                  onClick={() => {
          ...
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-[9999] p-4">
            <div className="bg-white rounded-xl shadow-2xl max-w-6xl w-full max-h-[90vh] overflow-y-auto">
              ...
              <div className="p-4">
                {uploadedImages.length > 0 && (
                  <>
                    <h4 className="font-semibold text-lg mb-3 text-indigo-600">Uploaded Images</h4>
                    <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 mb-6">
                      {uploadedImages.map((image, index) => (
                        <div
                          key={`upload-${index}`}
                          onClick={() => selectedProject && assignImage(selectedProject, image)}
                          className="cursor-pointer hover:opacity-80 transition-opacity border-2 border-indigo-300 rounded-lg overflow-hidden bg-indigo-50"
                        >
                          <img
                            src={image}
