...
export async function GET(request: NextRequest) {
  return NextResponse.json({ 
    status: 'WhatsApp Lead Capture webhook is ready!',
    timestamp: new Date().toISOString()
  });
}

// POST request to handle incoming messages and capture leads
export async function POST(request: NextRequest) {
  try {
    ...
    console.log(`📱 WhatsApp lead from ${profileName} (${phoneNumber}): ${messageBody}`);
    
    // Detect language and investment intent
    ...
    await captureWhatsAppLead(phoneNumber, profileName, messageBody, detectedLanguage);
    ...
    console.log(`✅ Lead response sent: ${message.sid}`);
