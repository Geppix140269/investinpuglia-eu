// PATH: components/trullo/TrulloChatbot.test.tsx
// THIS IS THE TEST VERSION WITH ENHANCEMENTS
'use client'
import React, { useState, useEffect, useRef } from 'react';
import { Language, TrulloChatbotProps, Message } from './types';
import { translations } from './constants/translations';
import { useChat } from './hooks/useChat.test'; // USING TEST VERSION
...
export default function TrulloChatbot({ language = 'en' }: TrulloChatbotProps) {
  ...
  const {
    ...
    sendMessage,
    closeChat,
    enhancedProfile // GET ENHANCED PROFILE FOR TESTING
  } = useChat(isOpen, currentLang) as any;
    ...
    <>
      ...
      {isOpen && (
        ...
        >
          ...
          {/* ============ ENHANCEMENT: Trust Score Display (TEST MODE) ============ */}
          {enhancedProfile && enhancedProfile.trustScore > 0 && (
            <div className="bg-gradient-to-r from-purple-50 to-emerald-50 px-4 py-2">
              <div className="flex items-center justify-between">
                <span className="text-xs text-gray-600">Trust Level (TEST)</span>
                <span className={`text-xs font-bold ${
                  enhancedProfile.engagementLevel === 'hot' ? 'text-green-600' :
                  enhancedProfile.engagementLevel === 'warm' ? 'text-yellow-600' :
                  'text-gray-600'
                }`}>
                  {enhancedProfile.engagementLevel === 'hot' ? '🔥 Hot Lead' :
                   enhancedProfile.engagementLevel === 'warm' ? '🌡️ Warming Up' :
                   '❄️ Building Trust'} ({enhancedProfile.trustScore}%)
                </span>
              </div>
              <div className="w-full bg-gray-200 rounded-full h-2 mt-1">
                <div 
                  className="bg-gradient-to-r from-purple-600 to-emerald-600 h-2 rounded-full transition-all duration-500"
                  style={{ width: `${enhancedProfile.trustScore}%` }}
                />
              </div>
              {enhancedProfile.email && (
                <div className="text-xs text-gray-600 mt-1">
                  📧 {enhancedProfile.email} | 💰 {enhancedProfile.investmentBudget || 'Not specified'}
                </div>
              )}
            </div>
          )}
